...
function loadGame() {
  ...
  if (typeof gameSelectedWord === "string") {
    ...
    // Set
    $("#textGuess").html(
      "<div style='font-size:70%;padding-bottom:5px;'>" +
        splittedWord[0] +
        "</div>" +
...
function addMessage(data, msg) {
  ...
  if (command == "/say" || command == "/ngomong" || command == "/dit") {
    // TTS
  ...
  if (mode == "winner") {
    addContent(
      `<div style="text-align:center;font-size: 1.25rem;">
                ...
                <div style='padding-bottom:.5rem;font-weight: bold;'>` +
        userName +
        `</div>
                <div>
                    <img src="` +
        userAvatar +
        `" style="width:128px;height:128px;border-radius: 15px;"/>
                </div>
            </div>`
    );
  } else {
...
function addGift(data) {
  ...
  if (giftTotal >= 10) {
    ...
    // Add
    addContent(
      `<div style="text-align:center;font-size: 1.25rem;"><div style='padding-bottom:.5rem;'>Thank you <span style='font-weight: bold;'>` +
        userName +
        `!</span></div>
            <div style='font-weight: bold;padding-bottom:.5rem;'><img src="` +
        giftPictureUrl +
        `" style="width:32px;height:32px;"/> Sent ` +
        giftName +
        `</div>
            x` +
        giftRepeat.toLocaleString() +
        ` worth ` +
